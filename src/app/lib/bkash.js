...
export const getBkashIdToken = async () => {
    try {
        ...
        if (bkashTokenTTL <= 600 &&
            ...
            if (!result.id_token) {
                throw new AppError(httpStatus.BAD_GATEWAY, "Bkash ID token not received from refresh API");
            }
        ...
        // ==========================================
        if (!result.id_token) {
            throw new AppError(httpStatus.BAD_GATEWAY, "Bkash ID token not received");
        }
        // ==========================================
        // Refresh Token check
        // ==========================================
        if (!result.refresh_token) {
            throw new AppError(httpStatus.BAD_GATEWAY, "Bkash refresh token not received");
        }
        // ==========================================
        ...
        throw new AppError(httpStatus.INTERNAL_SERVER_ERROR, error.message || "Bkash token error");
    }
};
