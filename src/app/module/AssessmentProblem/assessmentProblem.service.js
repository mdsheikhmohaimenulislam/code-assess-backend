...
const createAssessmentProblem = async (userId, userRole, assessmentId, payload) => {
    ...
    // Validate marks
    if (marks <= 0) {
        throw new AppError(httpStatus.BAD_REQUEST, "Marks must be greater than 0");
    }
    // Validate order
    if (order <= 0) {
        throw new AppError(httpStatus.BAD_REQUEST, "Order must be greater than 0");
    }
    // Find assessment
    ...
    if (userRole === Role.COMPANY &&
        ...
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to manage this assessment");
    }
    // Only DRAFT assessment can be modified
    if (assessment.status !== AssessmentStatus.DRAFT) {
        throw new AppError(httpStatus.BAD_REQUEST, "Problems can only be added to a draft assessment");
    }
    // Find problem
    ...
    const existingProblem = await prisma.assessmentProblem.findUnique({
        where: {
    ...
    if (existingProblem) {
        throw new AppError(httpStatus.CONFLICT, "This problem is already added to the assessment");
    }
    // Check duplicate order
    const existingOrder = await prisma.assessmentProblem.findUnique({
        where: {
            assessmentId_order: {
                assessmentId,
                order,
    ...
    if (existingOrder) {
        throw new AppError(httpStatus.CONFLICT, `Order ${order} is already used in this assessment`);
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        const marksResult = await tx.assessmentProblem.aggregate({
            where: {
                assessmentId,
            },
            _sum: {
...
const getAssessmentProblems = async (assessmentId) => {
    const assessment = await prisma.assessment.findUnique({
        where: {
            id: assessmentId,
        },
...
const getAssessmentProblemById = async (id) => {
    const result = await prisma.assessmentProblem.findUnique({
        where: {
            id,
        },
        select: {
...
const updateAssessmentProblem = async (userId, userRole, id, payload) => {
    ...
    if (!id) {
        throw new AppError(httpStatus.BAD_REQUEST, "Assessment problem ID is required");
    }
    ...
    if (assessment.status !== AssessmentStatus.DRAFT) {
        throw new AppError(httpStatus.BAD_REQUEST, "Problems can only be updated in a draft assessment");
    }
    // Check duplicate order
    if (order !== undefined &&
    ...
    const result = await prisma.$transaction(async (tx) => {
        const updated = await tx.assessmentProblem.update({
            where: {
...
const deleteAssessmentProblem = async (userId, userRole, id) => {
    // Check authentication
    if (!userId) {
        throw new AppError(httpStatus.UNAUTHORIZED, "Unauthorized");
    }
    ...
    // Only draft assessment can be updated
    if (assessment.status !== AssessmentStatus.DRAFT) {
        throw new AppError(httpStatus.BAD_REQUEST, "Problems can only be deleted from a draft assessment");
    }
    // Delete + recalculate total marks
    const result = await prisma.$transaction(async (tx) => {
        // Delete assessment problem
        const deleted = await tx.assessmentProblem.delete({
            where: {
                id: assessmentProblem.id,
            },
            select: {
...
//# sourceMappingURL=assessmentProblem.service.js.map