...
const createAssessment = async (userId, userRole, payload) => {
    // 1. Check user role
    if (userRole !== Role.ADMIN && userRole !== Role.COMPANY) {
        throw new AppError(403, "You do not have permission to create an assessment");
    }
    ...
    if (userRole === Role.COMPANY && company.userId !== userId) {
        throw new AppError(403, "You can only create an assessment for your own company");
    }
...
const getAssessments = async (query) => {
    ...
    const status = typeof query.status === "string"
        ...
        : undefined;
    const accessType = typeof query.accessType === "string"
        ? query.accessType.trim().toUpperCase()
        : undefined;
    const companyId = typeof query.companyId === "string" ? query.companyId.trim() : undefined;
    // Validate status
    if (status &&
    ...
    if (accessType &&
        !Object.values(AssessmentAccessType).includes(accessType)) {
        throw new AppError(httpStatus.BAD_REQUEST, "Invalid assessment access type");
    }
    // Validate company ID
    if (companyId) {
        const company = await prisma.companyProfile.findUnique({
    ...
    const requestedSort = typeof query.sortBy === "string" ? query.sortBy : "createdAt";
...
// Update Assessment
const updateAssessment = async (userId, userRole, assessmentId, payload) => {
    // Check user
    if (!userId) {
    ...
    if (userRole === Role.COMPANY && existingAssessment.createdById !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You do not have permission to update this assessment");
    }
    // Only DRAFT assessment can be updated
    if (existingAssessment.status !== AssessmentStatus.DRAFT) {
        throw new AppError(httpStatus.BAD_REQUEST, "Only draft assessment can be updated");
    }
    ...
    if (passingMarks > totalMarks) {
        throw new AppError(httpStatus.BAD_REQUEST, "Passing marks cannot be greater than total marks");
    }
    // Date validation
    ...
    if (accessType === AssessmentAccessType.PAID) {
        const finalPrice = payload.price !== undefined ? payload.price : existingAssessment.price;
        if (finalPrice === null || finalPrice === undefined) {
            throw new AppError(httpStatus.BAD_REQUEST, "Price is required for paid assessment");
        }
        if (Number(finalPrice) <= 0) {
            throw new AppError(httpStatus.BAD_REQUEST, "Paid assessment price must be greater than 0");
        }
    }
    ...
    if (accessType === AssessmentAccessType.FREE &&
        ...
        payload.price !== null &&
        payload.price !== 0) {
        throw new AppError(httpStatus.BAD_REQUEST, "Free assessment cannot have a price");
    }
    // Update assessment
...
//  Delete Assessment
const deleteAssessment = async (userId, userRole, assessmentId) => {
    // Check user
    if (!userId) {
        throw new AppError(httpStatus.UNAUTHORIZED, "Unauthorized");
    }
    ...
    if (userRole === Role.COMPANY && existingAssessment.createdById !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You do not have permission to delete this assessment");
    }
    // Only draft assessment can be deleted
    if (existingAssessment.status !== AssessmentStatus.DRAFT) {
        throw new AppError(httpStatus.BAD_REQUEST, "Only draft assessment can be deleted");
    }
    // Delete assessment
...
const updateAssessmentStatus = async (userId, userRole, assessmentId, status) => {
    if (!userId) {
        throw new AppError(httpStatus.UNAUTHORIZED, "Unauthorized");
    }
    if (!assessmentId) {
    ...
    if (userRole === Role.COMPANY && assessment.createdById !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You do not have permission to manage this assessment");
    }
    // Publish
    if (status === AssessmentStatus.PUBLISHED) {
        if (assessment.status !== AssessmentStatus.DRAFT) {
            throw new AppError(httpStatus.BAD_REQUEST, "Only draft assessment can be published");
        }
        ...
        if (problemCount === 0) {
            throw new AppError(httpStatus.BAD_REQUEST, "Assessment must contain at least one problem");
        }
        if (assessment.accessType === AssessmentAccessType.PAID &&
            (!assessment.price || Number(assessment.price) <= 0)) {
            throw new AppError(httpStatus.BAD_REQUEST, "Paid assessment must have a valid price");
        }
    }
    // Cancel
    if (status === AssessmentStatus.CANCELLED) {
        if (assessment.status === AssessmentStatus.COMPLETED ||
            ...
            throw new AppError(httpStatus.BAD_REQUEST, "This assessment cannot be cancelled");
        }
    }
    // Complete
    if (status === AssessmentStatus.COMPLETED) {
        if (assessment.status !== AssessmentStatus.ONGOING) {
            throw new AppError(httpStatus.BAD_REQUEST, "Only ongoing assessment can be completed");
        }
    }
    const updatedAssessment = await prisma.assessment.update({
        where: {
