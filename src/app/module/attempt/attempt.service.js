import { AssessmentStatus, AttemptStatus, InvitationStatus, Role, } from "../../../generated/prisma/enums.js";
import { prisma } from "../../lib/prisma.js";
import httpStatus from "http-status";
import { AppError } from "../../utils/AppError.js";
const createAttempt = async (userId, assessmentId) => {
    ...
    if (assessment.status !== AssessmentStatus.PUBLISHED &&
        ...
        throw new AppError(httpStatus.BAD_REQUEST, "This assessment is not available for attempting");
    }
    //  4. Check assessment start time
    const now = new Date();
    if (assessment.startTime && now < assessment.startTime) {
        throw new AppError(httpStatus.BAD_REQUEST, "Assessment has not started yet");
    }
    ...
    if (!invitation) {
        throw new AppError(httpStatus.FORBIDDEN, "You must accept the invitation before starting the assessment");
    }
    ...
    if (existingAttempt) {
        if (existingAttempt.status === AttemptStatus.IN_PROGRESS) {
            throw new AppError(httpStatus.CONFLICT, "You already have an active attempt for this assessment");
        }
        throw new AppError(httpStatus.CONFLICT, "You have already attempted this assessment");
    }
...
const getAttempts = async ({ userId, userRole, page, limit, }) => {
...
const getAttemptById = async (userId, userRole, attemptId) => {
...
const submitAttempt = async (userId, attemptId) => {
    ...
    if (attempt.candidateId !== candidate.id) {
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to submit this attempt");
    }
    // 4. Already submitted
    ...
    if (attempt.status !== AttemptStatus.IN_PROGRESS) {
        throw new AppError(httpStatus.BAD_REQUEST, "Only an in-progress attempt can be submitted");
    }
    const now = new Date();
    // 7. Check expiration
