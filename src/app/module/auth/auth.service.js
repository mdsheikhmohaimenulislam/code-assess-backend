...
const verifyEmail = async (payload) => {
    try {
        ...
        if (isUserExist?.status === "INACTIVE") {
            throw new AppError(httpStatus.FORBIDDEN, "Your account is inactive. Please contact support.");
        }
        // OTP
        const otpKey = `User-registration-otp:${email}`;
...
const loginUser = async (payload) => {
    ...
    if (user.password === null && user.googleId !== null) {
        throw new AppError(httpStatus.CONFLICT, "User Already Has Account Registered With Google. Try To Login With Google.");
    }
