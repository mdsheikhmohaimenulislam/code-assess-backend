...
const createInvitation = async (userId, userRole, payload) => {
    const { assessmentId, candidateId, userId: candidateUserId, email, expiresAt, } = payload;
    // Check assessment
    ...
    if (assessment.status !== AssessmentStatus.PUBLISHED &&
        ...
        throw new AppError(httpStatus.BAD_REQUEST, "Candidates can only be invited to a published or ongoing assessment");
    }
    ...
    if (candidateUser.role !== Role.CANDIDATE) {
        throw new AppError(httpStatus.BAD_REQUEST, "The selected user is not a candidate");
    }
    // Candidate account must be active
    if (candidateUser.status !== UserStatus.ACTIVE) {
        throw new AppError(httpStatus.BAD_REQUEST, "Candidate account is not active");
    }
    ...
    if (candidate.userId !== candidateUserId) {
        throw new AppError(httpStatus.BAD_REQUEST, "Candidate profile does not belong to this user");
    }
    // Email must match candidate account
    if (candidateUser.email.toLowerCase() !== email.toLowerCase()) {
        throw new AppError(httpStatus.BAD_REQUEST, "Email does not match candidate account");
    }
    ...
    if (existingInvitation) {
        throw new AppError(httpStatus.CONFLICT, "This candidate has already been invited to this assessment");
    }
    // Create invitation
...
const getInvitationById = async (userId, userRole, invitationId) => {
    ...
    if (userRole === Role.CANDIDATE && invitation.userId !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to view this invitation");
    }
    /**
...
const acceptInvitation = async (userId, invitationId) => {
    ...
    if (invitation.userId !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to accept this invitation");
    }
    /**
     * Invitation must be pending
     */
    if (invitation.status !== InvitationStatus.PENDING) {
        throw new AppError(httpStatus.BAD_REQUEST, `Invitation is already ${invitation.status.toLowerCase()}`);
    }
    /**
     * Check expiration
     */
     ...
     */
    if (invitation.assessment.status === AssessmentStatus.CANCELLED ||
        invitation.assessment.status === AssessmentStatus.COMPLETED) {
        throw new AppError(httpStatus.BAD_REQUEST, "This assessment is no longer available");
    }
    /**
     * Accept invitation
     */
...
const rejectInvitation = async (userId, invitationId) => {
     ...
     */
    if (invitation.userId !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to reject this invitation");
    }
    /**
     * Invitation must be pending
     */
...
const deleteInvitation = async (userId, userRole, invitationId) => {
    ...
    if (userRole === Role.COMPANY &&
        invitation.assessment.company.userId !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to delete this invitation");
    }
    /**
     * Only pending invitation can be deleted
     */
    if (invitation.status !== InvitationStatus.PENDING) {
        throw new AppError(httpStatus.BAD_REQUEST, "Only pending invitations can be deleted");
    }
