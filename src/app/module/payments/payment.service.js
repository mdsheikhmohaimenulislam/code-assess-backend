...
// CREATE PAYMENT
// ======================================================
const createPayment = async (userId, assessmentId) => {
    if (!userId) {
    ...
    if (assessment.accessType !== "PAID") {
        throw new AppError(httpStatus.BAD_REQUEST, "This assessment does not require payment");
    }
    // Check price
    if (assessment.price === null ||
    ...
    if (existingPayment?.status === PaymentStatus.PAID) {
        throw new AppError(httpStatus.CONFLICT, "This assessment has already been paid");
    }
    // Get bKash token
...
const executePayment = async (paymentID) => {
    ...
    if (result.statusCode === "0000") {
        if (!result.trxID) {
            throw new AppError(httpStatus.BAD_GATEWAY, "bKash transaction ID not received");
        }
