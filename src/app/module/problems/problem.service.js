...
const createProblem = async (userId, payload) => {
    const { title, description, type, difficulty, category, inputFormat, outputFormat, constraints, timeLimit, memoryLimit, options, } = payload;
    // MCQ Validation
    ...
    if (type === ProblemType.CODING && options) {
        throw new AppError(httpStatus.BAD_REQUEST, "Options are only allowed for MCQ problems");
    }
    // ==========================================
    // Currently Only Coding Problems Allowed
    // ==========================================
    if (type !== ProblemType.CODING) {
        throw new AppError(httpStatus.BAD_REQUEST, "Only coding problems are allowed at this time");
    }
    // ==========================================
...
const updateProblem = async (userId, userRole, problemId, payload) => {
    ...
    if (Object.keys(payload).length === 0) {
        throw new AppError(400, "At least one field is required to update the problem");
    }
...
const deleteProblem = async (userId, userRole, problemId) => {
    const existingProblem = await prisma.problem.findUnique({
        where: {
            id: problemId,
        },
