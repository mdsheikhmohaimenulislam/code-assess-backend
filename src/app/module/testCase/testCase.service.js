...
const createTestCase = async (userId, userRole, problemId, payload) => {
    ...
    if (userRole === Role.COMPANY && problem.createdById !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to add a test case to this problem");
    }
    // Test cases are only for coding problems
    if (problem.type !== ProblemType.CODING) {
        throw new AppError(httpStatus.BAD_REQUEST, "Test cases can only be added to coding problems");
    }
    // Create test case
...
const getTestCase = async (userId, userRole, testCaseId) => {
    ...
    if (userRole === Role.COMPANY && testCase.problem.createdById !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to view this test case");
    }
    // ==========================================
...
const updateTestCase = async (userId, userRole, id, payload) => {
    ...
    if (userRole === Role.COMPANY && testCase.problem.createdById !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to update this test case");
    }
...
const deleteTestCase = async (userId, userRole, id) => {
    ...
    if (userRole === Role.COMPANY && testCase.problem.createdById !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to delete this test case");
    }
